...
class GenerateTicketRepository @Inject constructor(private val apiService: ApiService) {



     fun generateTicket(cipher: EncryptedRequest):
             Flow<ResponseAPI<EncryptedResponse>> = flow {
             try {
                 val apiResponse = apiService.generateTicket(cipher)
